...
function TransactionDetail({ transaction, loading }) {
      ...
      timeline.push({
        ...
        children: (
          <div>
            ...
            <br />
            <Text type="secondary">{formatDateTime(transaction.init_time)}</Text>
            <br />
      ...
      timeline.push({
        ...
        children: (
          <div>
            ...
            <br />
            <Text type="secondary">{formatDateTime(transaction.send_time)}</Text>
            {transaction.error && (
              <>
                <br />
      ...
      timeline.push({
        ...
        children: (
          <div>
            ...
            <br />
            <Text type="secondary">{formatDateTime(transaction.res_time)}</Text>
          </div>
        )
      })
    }
  ...
  return (
    <div className={styles.transactionDetail}>
      <Tabs activeKey={activeTab} onChange={setActiveTab} type="card" className={styles.mainTabs}>
        <TabPane 
          tab={
            <Space>
              <InfoCircleOutlined />
        ...
        >
          <div className={styles.tabContent}>
              ...
              <Col xs={24} sm={12} md={6}>
                <Card size="small" className={styles.statusCard}>
                  <div className={styles.statusCardContent}>
                    ...
                    <DirectionBadge direction={transaction.direction} size="large" />
                  </div>
                </Card>
              </Col>
              <Col xs={24} sm={12} md={6}>
                ...
                <Card size="small" className={styles.dataCard}>
                  <div className={styles.statusCardContent}>
                    <Space>
                      ...
                      <Text type="secondary">JSON Data</Text>
                    </Space>
                    <Space>
                      <Tag color={transaction.json_data ? 'green' : 'red'}>
                        {transaction.json_data ? 'Available' : 'Not Available'}
                      </Tag>
                      {transaction.json_data && (
                        <Text type="secondary" style={{ fontSize: '11px' }}>
                          {transaction.json_data.length} chars
                        </Text>
                      )}
                    </Space>
                  </div>
                ...
                <Card size="small" className={styles.dataCard}>
                  <div className={styles.statusCardContent}>
                    <Space>
                      ...
                      <Text type="secondary">XML Data</Text>
                    </Space>
                    <Space>
                      <Tag color={transaction.xml_data ? 'green' : 'red'}>
                        {transaction.xml_data ? 'Available' : 'Not Available'}
                      </Tag>
                      {transaction.xml_data && (
                        <Text type="secondary" style={{ fontSize: '11px' }}>
                          {transaction.xml_data.length} chars
                        </Text>
                      )}
                    </Space>
                  </div>
            ...
            {transaction.error && transaction.error !== 0 && (
              <Alert
                type="error"
                message={`Error ${transaction.error}: ${transaction.error_description || 'Unknown error'}`}
            ...
            <Card title="Transaction Information" size="small">
              <Descriptions column={{ xs: 1, sm: 2, md: 3 }} size="small">
                <Descriptions.Item label="Message ID">
                  <Text code>{transaction.message_id || 'N/A'}</Text>
                </Descriptions.Item>
                <Descriptions.Item label="Reference">
                  <Text code>{transaction.reference_ || 'N/A'}</Text>
                </Descriptions.Item>
                ...
                <Descriptions.Item label="File Name">
                  <Text code>{transaction.file_name || 'N/A'}</Text>
                </Descriptions.Item>
                <Descriptions.Item label="Response File">
                  <Text code>{transaction.res_file_name || 'N/A'}</Text>
                </Descriptions.Item>
            ...
            {/* Session Information */}
             <Card title="Session Information" size="small" style={{ marginTop: 16 }}>
              ...
              <div>
                ...
                <Row gutter={16}>
                  ...
                  <Col span={8}>
                    <Statistic
                      title="Status"
                      value={transaction.error ? 'Failed' : 'Success'}
                      valueStyle={{ color: transaction.error ? '#ff4d4f' : '#52c41a' }}
                    />
                  </Col>
                </Row>
              </div>
        ...
        <TabPane 
          tab={
            <Space>
              <FileTextOutlined />
              <span>JSON Data</span>
              {transaction.json_data && <Tag color="green" size="small">Available</Tag>}
            </Space>
          } 
          key="json"
        >
        ...
        <TabPane 
          tab={
            <Space>
              <CodeOutlined />
              <span>XML Data</span>
              {transaction.xml_data && <Tag color="green" size="small">Available</Tag>}
            </Space>
          } 
          key="xml"
        >
