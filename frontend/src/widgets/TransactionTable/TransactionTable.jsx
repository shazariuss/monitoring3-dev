...
function TransactionTable() {
  ...
  const getSuccessCount = () => {
    ...
    return transactions.data.filter(t => t.state === 9 && (!t.error || t.error === 0)).length
  }
  ...
  const getPendingCount = () => {
    ...
    return transactions.data.filter(t => [1, 2, 3].includes(t.state)).length
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'message_id',
      width: 150,
      render: (text) => text ? <Text code>{text}</Text> : <Text type="secondary">N/A</Text>
    },
    {
      title: 'Type',
      dataIndex: 'type',
    ...
    <div className={styles.transactionTable}>
      ...
      <Card 
          ...
          <Space>
            ...
            {lastUpdate && (
              <Text type="secondary" style={{ fontSize: '12px' }}>
                | Last updated: {dayjs(lastUpdate).format('HH:mm:ss DD/MM/YYYY')}
              </Text>
            )}
          </Space>
        }
